...
const ARCHIVO_ESTADISTICAS = path.join(__dirname, "..", "data", "estadisticas.json");
...
// Crear archivo de estadísticas si no existe
async function inicializarEstadisticas() {
  try {
    await leerArchivoJSON(ARCHIVO_ESTADISTICAS, ESTRUCTURA_ESTADISTICAS);
    log("📊 Sistema de estadísticas inicializado");
  } catch (error) {
    log(`❌ Error inicializando estadísticas: ${error.message}`, "ERROR");
  }
}

// Leer estadísticas (ahora usa el sistema seguro)
async function leerEstadisticas() {
  return await leerArchivoJSON(ARCHIVO_ESTADISTICAS, ESTRUCTURA_ESTADISTICAS);
}
...
// Obtener semana en formato YYYY-WW
...
async function registrarMensaje(userId, tipo = "recibido") {
  const fecha = getFechaHoy();
  
  try {
    await modificarArchivoJSON(ARCHIVO_ESTADISTICAS, ESTRUCTURA_ESTADISTICAS, (stats) => {
      ...
      // Registrar usuario único por día
      ...
      // Agregar usuario si no existe ya
...
// Registrar handoff solicitado
async function registrarHandoff(userId, razon = "solicitado") {
  const fecha = getFechaHoy();
  ...
  try {
    await modificarArchivoJSON(ARCHIVO_ESTADISTICAS, ESTRUCTURA_ESTADISTICAS, (stats) => {
      ...
      // También incrementar en mensajes
      if (!stats.mensajes[fecha]) {
        stats.mensajes[fecha] = { recibidos: 0, enviados: 0, handoffs: 0 };
      }
...
// Registrar intento de hijacking
async function registrarHijacking(userId, tipo) {
  const fecha = getFechaHoy();
...
async function registrarBusqueda(consulta, resultados = 0) {
  const fecha = getFechaHoy();
  ...
  try {
    await modificarArchivoJSON(ARCHIVO_ESTADISTICAS, ESTRUCTURA_ESTADISTICAS, (stats) => {
      ...
      // Guardar consulta (máximo 100 por día para no llenar el archivo)
      if (stats.busquedas[fecha].consultas.length < 100) {
        stats.busquedas[fecha].consultas.push({
          consulta: consulta.substring(0, 50), // Limitar longitud
          resultados,
          timestamp: Date.now()
        });
      }
...
async function getEstadisticasUltimosDias(dias = 7) {
  const stats = await leerEstadisticas();
  const resultado = [];
...
async function getResumenGeneral() {
  ...
  const ultimos7Dias = await getEstadisticasUltimosDias(7);
  ...
  // Calcular usuarios únicos de la semana (sin duplicados)
...
async function getConsultasFrecuentes(dias = 7) {
  const stats = await leerEstadisticas();
  const consultas = {};
  
  // Recopilar consultas de los últimos días
