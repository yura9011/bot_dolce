...
const { esAdmin, getRolAdmin, procesarComandoAdmin } = require("./admin-commands");
...
class AgentManager {
  ...
  async responderBot(message, texto) {
    ...
    notificarDashboard(message.from);
    return respuesta;
  }
...
function notificarDashboard(userId) {
  ...
  req.on('error', () => {}); // silencioso si dashboard no está corriendo
  req.write(data);
  req.end();
}
