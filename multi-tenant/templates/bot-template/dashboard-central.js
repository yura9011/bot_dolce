...
const PORT = process.env.PORT || process.env.DASHBOARD_CENTRAL_PORT || 3000;
...
app.get('/api/phone-aliases', (req, res) => {
  const aliasesPath = process.env.ALIASES_PATH || path.join(__dirname, 'config', 'phone-aliases.json');
...
function getNotasPath(agentId) {
  const config = loadAgentsConfig();
  const agent = config.agents.find(a => a.id === agentId);
  if (!agent) return null;
  return path.join(process.cwd(), agent.paths.data, 'notas.json');
}

// Cargar configuración de agentes
function loadAgentsConfig() {
  const configPath = process.env.CONFIG_PATH || path.join(__dirname, 'config', 'agents.json');
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`📊 Dashboard disponible en: http://0.0.0.0:${PORT}`);
